...
function App() {
  ...
  const handleKeyPress = (keyCode, key) => {
    ...
    if (numbers.includes(key)) {
      ...
      if (tempHistroy.length > 20) tempHistroy = tempHistroy.splice(0, 1);
      tempHistroy.push(expression);
      setHistory(tempHistroy);
    }
  };
  ...
  return (
    ...
    >
      <div className="calculator">
        <div className="output_navbar">
          <div
            className="output_navbar_toggle"
            onClick={() => setisDarkmode(!isDarkmode)}
          >
            <div
              className={`output_navbar_toggle_circle ${
                isDarkmode ? "output_navbar_toggle_circle_active" : ""
              }`}
            ></div>
          </div>
